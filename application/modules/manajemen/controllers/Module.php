        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (isset($session)) {
                ...
                foreach ($dataList as $key => $dl) {
                    ...
                    $row = array();
                    $row[] = '
                    <button type="button" class="btn mb-1 btn-success btn-sm btnRules px-2 py-1" data-id="'.$this->encryption->encrypt($dl['id_module']).'" title="Konfigurasi"><i class="fa fa-spin fa-cog fa-lg"></i></button>
                    <button type="button" class="btn btn-primary btn-sm mb-1 px-2 py-1 btnEdit" data-id="'.$this->encryption->encrypt($dl['id_module']).'" title="Edit data"><i class="fa fa-pencil-square fa-lg"></i></button>
                    <button type="button" class="btn mb-1 btn-danger btn-sm btnDelete px-2 py-1" data-id="'.$this->encryption->encrypt($dl['id_module']).'" title="Hapus data"><i class="fa fa-trash-o fa-lg"></i></button>';
                    $row[] = $no;
                    $row[] = $dl['label_module'];
                    $row[] = $dl['nama_module'];
                    $row[] = $dl['url_module'];
