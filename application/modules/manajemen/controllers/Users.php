            ...
            if(isset($session)) {
                ...
                foreach ($dataUser as $key => $u) {
                    ...
                    foreach ($arrGroup as $g) {
                        $nm_group .= '<li style="white-space:nowrap;">'.$g.'</li>';
                    }
                    $nm_group .= '</ul>';
                    $password = ($this->app_loader->is_super()) ? '<li><strong>Password :</strong> '.$u['pass_plain'].'</li>' : '';

                    $row[] = '<div class="form-check checkbox checkbox-primary mb-0">
                                <input class="form-check-input checkbox_animated" type="checkbox" name="checkid[]" id="u_'.$u['token'].'" value="'.$u['token'].'">
                                <label class="form-check-label" for="u_'.$u['token'].'"></label>
                            </div>';
                    $row[] = $no;
                    $row[] = '<ul class="list-unstyled" style="margin-bottom:0px;">'.
                                '<li style="white-space:nowrap;"><strong>Username :</strong> '.$u['username'].'</li>'.
                                $password.
                                '<li style="white-space:nowrap;"><strong>Nama :</strong> '.$u['fullname'].'</li>'.
                              '</ul>';
                    $row[] = $nm_group;
                    $row[] = ($u['unit_id_name']) ? $u['unit_id_name'] : "-";
                    $row[] = convert_blokir($u['blokir']);
                    $row[] = convert_status($u['id_status']);
                    $row[] = '<button type="button" class="btn btn-primary btn-sm mb-1 px-2 py-1 btnEdit" data-id="'.$u['token'].'" title="Edit data user"><i class="fa fa-pencil-square fa-lg"></i> </button>';
                    $data[] = $row;
                }
                $output = array(
