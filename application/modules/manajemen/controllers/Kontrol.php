        ...
        if(!$this->input->is_ajax_request()) {
            ...
            if(isset($session)){
                ...
                foreach ($dataList as $key => $dl) {
                    $no++;
                    $row = array();
                    $row[] = '<button type="button" class="btn btn-primary btn-sm mb-1 px-2 py-1 btnEdit" data-id="'.$this->encryption->encrypt($dl['id_kontrol']).'" title="Edit data"><i class="fa fa-pencil-square fa-lg"></i></button>
                    <button type="button" class="btn mb-1 btn-danger btn-sm btnDelete px-2 py-1" data-id="'.$this->encryption->encrypt($dl['id_kontrol']).'" title="Hapus data"><i class="fa fa-trash-o fa-lg"></i></button>';
                    $row[] = $no;
