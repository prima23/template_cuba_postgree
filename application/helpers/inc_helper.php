...
if (!function_exists('error_message')) {
    function error_message($type, $label, $message) {
		...
		// 	      </div>';
        $error = '<div class="alert alert-'.$type.' inverse alert-dismissible fade show">
                    <i class="icon-info-alt"></i>
                    <p id="pesanSistem">'.$message.'</p>
...
if (!function_exists('status')) {
    function status($group = 1)
    {
        $ci = &get_instance();
        $ci->load->database();
        $datas = $ci->db->get_where('xi_sa_status', array('kel_status' => $group))->result();
        if(!$datas){
            return array('' => 'Pilih Status');
        }
        $status = array('' => 'Pilih Status');
        foreach($datas as $data){
            $status[$data->id_status] = $data->nm_status;
        }
        return $status;
    }
}
...
if (!function_exists('convert_status')) {
    function convert_status($id_status) {
        $status = array(
            0 => '<span class="badge rounded-pill badge-danger">Tidak aktif</span>',
            1 => '<span class="badge rounded-pill badge-success">Aktif</span>',
        );
