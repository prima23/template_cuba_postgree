        ...
        if(!$this->input->is_ajax_request()) {
            ...
            if(isset($session)){
                ...
                foreach ($dataList as $key => $dl) {
                    $no++;
                    $row = array();
                    $row[] = '<button type="button" class="btn btn-primary btn-sm mb-1 px-2 py-1 btnEdit" data-id="'.$this->encryption->encrypt($dl['id_fungsi']).'" title="Edit data"><i class="fa fa-pencil-square fa-lg"></i></button>
                    <button type="button" class="btn mb-1 btn-danger btn-sm btnDelete px-2 py-1" data-id="'.$this->encryption->encrypt($dl['id_fungsi']).'" title="Hapus data"><i class="fa fa-trash-o fa-lg"></i></button>';
                    $row[] = $no;
                    $row[] = $dl['label_fungsi'];
                    $row[] = $dl['nama_fungsi'];
                    $row[] = $dl['url_fungsi'];
                    $row[] = '<span class="badge rounded-pill badge-primary">'.$dl['jenis_fungsi'].'</span>';
    ...
    public function delete() {
        if(!$this->input->is_ajax_request()) {
            ...
            if(!empty($session) AND !empty($funcId)) {
                ...
                if($data['response'] == 'ERROR') {
                    $result = array('status' => 'RC404', 'message' => 'Proses hapus data fungsi gagal, karena data tidak ditemukan', 'csrfHash' => $csrfHash);
                } else if($data['response'] == 'ERRDATA') {
                    $result = array('status' => 'RC404', 'message' => 'Proses hapus data fungsi dengan nama '.$data['nama'].' gagal, karena data sedang digunakan', 'csrfHash' => $csrfHash);
                } else if($data['response'] == 'SUCCESS') {
                    $result = array('status' => 'RC200', 'message' => 'Proses hapus data fungsi dengan nama '.$data['nama'].' sukses', 'csrfHash' => $csrfHash);
                }
            } else {
                $result = array('status' => 0, 'message' => 'Proses hapus data fungsi gagal, mohon coba kembali', 'csrfHash' => $csrfHash);
            }
            $this->output->set_content_type('application/json')->set_output(json_encode($result));
        }
    }
